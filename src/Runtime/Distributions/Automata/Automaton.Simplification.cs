...
namespace Microsoft.ML.Probabilistic.Distributions.Automata
{
        ...
        where TThis : Automaton<TSequence, TElement, TElementDistribution, TSequenceManipulator, TThis>, new()
    {
        ...
        public bool IsDeterministic()
        {
            ...
            for (int stateId = 0; stateId < this.statesData.Count; ++stateId)
            {
                var state = this.statesData[stateId];
        ...
        public bool TryDeterminize(int maxStatesBeforeStop)
        {
            Argument.CheckIfInRange(
                ...
                "maxStatesBeforeStop",
                "The maximum number of statesData must be positive and not greater than the maximum number of statesData allowed in an automaton.");
        ...
        {
            if (this.statesData.Count > MaxStateCountBeforeSimplification || this.PruneTransitionsWithLogWeightLessThan != null)
            {
        ...
        {
            int[] oldToNewStateIdMapping = new int[this.statesData.Count];
            int newStateId = 0;
